...
const Sidebar = ({sidebar,setSidebar}) => {
  ...
  return (
    <div className={sidebar?'sideBar-show vh-100':'w-0 vh-100 position-fixed'}  style={{width:"5%",backgroundColor:"#2C3034",zIndex:"2"}}>
        <aside  className="vh-100 d-flex flex-column align-items-center shadow-lg p-3 mb-5 rounded" style={{backgroundColor:"#2C3034"}}>
        <div className='mt-2 w-100 d-flex justify-content-end'>
           {/* { sidebar?<span className=''><img className="w-25 mt-1 ms-2" src={logo} alt="logo" /></span>:null} */}
            <span className='btn p-1' onClick={()=> setSidebar(sidebar?false:true)} >{sidebar?<div className='btn-close btn-close-white text-light'/>:<CiMenuBurger size={30} className='text-light'/>}</span> 
